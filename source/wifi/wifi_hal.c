...
static int initNl80211(Netlink* nl) {
      ...
      if (genl_connect(nl->socket)) {
           fprintf(stderr, "Failed to connect tosocket.\n");
           nl_close(nl->socket);
      ...
      if (nl->id< 0) {
           fprintf(stderr, "Nl80211 interface not there.\n");
           nl_close(nl->socket);
           nl_socket_free(nl->socket);
           return -ENOENT;
      }
...
INT wifi_getApAssociatedDeviceRxStatsResult(INT radioIndex, mac_address_t *clientMacAddress, wifi_associated_dev_rate_info_rx_stats_t **stats_array, UINT *output_array_size, ULLONG *handle)
{
      ...
      nla_put(msg2, NL80211_ATTR_MAC, ETH_ALEN, mac_addr);
      nla_put_u32(msg2, NL80211_ATTR_IFINDEX, if_nametoindex(if_name));
      nl_send_auto(nl.socket, msg2);
      nl_cb_set(nl.cb1, NL_CB_VALID , NL_CB_CUSTOM, rxStatsInfo_callback, stats_array);
...
INT wifi_getApAssociatedDeviceTxStatsResult(INT radioIndex, mac_address_t *clientMacAddress, wifi_associated_dev_rate_info_tx_stats_t **stats_array, UINT *output_array_size, ULLONG *handle)
{
      ...
      nla_put_u32(msg2, NL80211_ATTR_IFINDEX, if_nametoindex(if_name));
      nl_send_auto(nl.socket, msg2);
      nl_cb_set(nl.cb1, NL_CB_VALID , NL_CB_CUSTOM, txStatsInfo_callback, stats_array);
